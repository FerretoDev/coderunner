...
sys.path.append(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))  # Ajuste de ruta robusto y portátil [web:21]
...
from game.pantalla_juego import PantallaJuego  # Pantalla que corre el juego principal (loop propio) [web:47]
from models.administrador import Administrador  # Lógica de autenticación de administrador [web:21]
from models.salon_fama import SalonFama  # Modelo para gestionar puntuaciones y récords [web:21]
from models.sistema_sonido import SistemaSonido  # Sistema centralizado de sonidos [web:47]
...
class Juego:
    ...
    def __init__(self):
        ...
        self._computadora = None  # Placeholder para IA/enemigos si se necesita a futuro [web:21]
        self._laberinto = None  # Placeholder para el mapa/escenario [web:21]
        self.sonido = SistemaSonido()  # Gestor de sonidos (cargar/reproducir efectos) [web:47]
        self.salon_fama = SalonFama()  # Gestor de puntajes para mostrar/guardar récords [web:21]
        ...
        self.clock = pygame.time.Clock()  # Reloj para controlar FPS si se usa un loop aquí [web:47]
    ...
    def iniciar(self):
        ...
        screen = pygame.display.set_mode((800, 600))  # Crea la Surface principal donde se dibuja [web:47]
        pygame.display.set_caption("CodeRunner")  # Título visible en la barra de la ventana [web:47]

        # Instancias de servicios que se comparten entre pantallas
        salon_fama = SalonFama()  # Mantiene los récords accesibles desde el menú y su pantalla [web:21]
        admin = Administrador("casa")  # Admin con clave por defecto “casa” para pruebas [web:21]
        ...
        while ejecutando:
            ...
            screen = pygame.display.set_mode((800, 600))  # Útil si otras pantallas modifican la ventana [web:47]
            pygame.display.set_caption("CodeRunner")  # Restituye el título principal si cambió [web:47]

            # Mostrar menú principal y esperar a que el usuario elija
            menu = MenuPrincipal(screen)  # Crea el menú con la Surface de la ventana actual [web:47]
            opcion = menu.ejecutar()  # Devuelve un número de opción elegido por el usuario [web:21]

            if opcion == 1:  # Iniciar Juego
                pantalla_inicio = PantallaIniciarJuego(screen)  # Pantalla para pedir el nombre del jugador [web:21]
                nombre = pantalla_inicio.ejecutar()  # Devuelve el nombre o None si canceló [web:21]

                if nombre:
                    # Inicia el juego principal con el nombre ingresado
                    pantalla = PantallaJuego(nombre)  # Pantalla del juego con su propio loop interno [web:47]
                    pantalla.ejecutar()  # Corre hasta que termine esa pantalla (regresa al menú) [web:47]

            elif opcion == 2:  # Salón de la Fama
                pantalla_salon = PantallaSalonFama(screen, salon_fama)  # Pasamos el gestor para leer datos [web:21]
                pantalla_salon.ejecutar()  # Muestra la tabla de récords y vuelve al menú al cerrar [web:21]

            elif opcion == 3:  # Administración
                pantalla_admin = PantallaAdministracion(screen)  # Pantalla que pide la clave al usuario [web:21]
                clave = pantalla_admin.ejecutar()  # Devuelve la clave ingresada o None si canceló [web:21]

                if clave and admin.autenticar(clave):  # Verifica credenciales de admin [web:21]
                    modal = MensajeModal(
                        screen,
                        "✅ Acceso Concedido",
                ...
                ejecutando = False  # Sale del loop principal y cierra la aplicación [web:47]
    ...
    def mostrar_estado(self):
        ...
        if self._jugador:  # Evita errores si aún no hay jugador activo [web:21]
            print(f"Jugador: {self._jugador['nombre']}")  # Nombre del jugador para seguimiento [web:21]
    ...
    def terminar(self):
        ...
        print("🎮 Juego terminado")  # Mensaje de cierre para indicar final del ciclo [web:21]
        if self._jugador:  # Muestra puntaje final si corresponde [web:21]
            print(f"Puntaje final: {self._jugador['puntaje']}")  # Diagnóstico final [web:21]
